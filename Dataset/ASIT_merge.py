...
def uniform_key_points_index(skeleton_data):
    ...
    output[0] = (skeleton_data[11]+skeleton_data[12])//2
    output[1] = skeleton_data[11]
    output[2] = skeleton_data[13]
    output[3] = skeleton_data[15]
    output[4] = skeleton_data[12]
    output[5] = skeleton_data[14]
    output[6] = skeleton_data[16]
    output[7] = (skeleton_data[5]+skeleton_data[6])//2
    output[8] = skeleton_data[0]
    output[9] = skeleton_data[5]
    output[10] = skeleton_data[7]
    output[11] = skeleton_data[9]
    output[12] = skeleton_data[6]
    output[13] = skeleton_data[8]
    output[14] = skeleton_data[10]
...
if __name__ == '__main__':
    root_path = '/media/imaginarium/a0c299ce-8eea-4f25-92a4-b572d215821b/dataset/AIST_Dance/'
    # output_folder = '/media/imaginarium/12T/MergeDataset/'
    output_folder = '/media/imaginarium/a0c299ce-8eea-4f25-92a4-b572d215821b/merge/'
    GT_path = root_path + 'annot/AIST_Dance.pkl'
        ...
        if len(save_frames) > 0 and current_frame_name != GT_file[i]['image'].split('/')[0]:
            ...
            if np.isnan(GT_joints).any() or np.isinf(GT_joints).any():
                ...
                if len(save_frames) < MAX_NUM:
                    ...
                    savePath = output_folder + 'data/AIST_Dance' + '_' + str(saveID) + '.pt'
                    output = torch.cat(save_tensor, dim=0)
                    torch.save(output.detach(), savePath)

                    GT_savePath = output_folder + 'GT/AIST_Dance' + '_' + str(saveID) + '.npy'
